...
class Apsfc
{
	...
	/**
	 * Initialize Polylang API so we can call some function prematurely its initialization.
	 *
	 * @return void
	 */
	public function premature_initialization()
	{
	...
	{
		/**
		 * Prematurely initialize Polylang API
		 */
		add_action('plugins_loaded', [$this, 'premature_initialization']);

		/**
		 * Load plugin text domain (l10n)
		 */
		add_action('plugins_loaded', [$this, 'pc_load_textdomain'], 0);
		$this->setup_settings();
	}
	...
	{
		if (function_exists('pll_current_language')) return pll_current_language() !== pll_default_language();
		return false;
	}
	...
	public function add_lang_to_customizer_previewer()
	{
		...
		wp_add_inline_script(
			$handle,
			sprintf(
				'var pcLangTrans = "' . __('Language', 'apsfc') . '";' .
					'PSPolyLang.init( %s );',
				wp_json_encode(
					[
						'url'              => $current_url,
